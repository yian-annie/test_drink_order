    ...
    async def on_message(self, message):
        ...
        if self.user.mentioned_in(message):
            ...
            if msg == 'ping':
                ...
                #從這裡開始接上 NLU 模型
                responseSTR = "我是預設的回應字串…你會看到這串字，肯定是出了什麼錯！"
                inputLIST = [msg]
                filterLIST = []
                ...
                if "ice" in resultDICT and "temperature" not in resultDICT:
                    ...
                    for k in range(0,len(resultDICT["amount"])):
                        responseSTR = responseSTR + "{} X {} ({}、{})\n".format(resultDICT["item"][k],resultDICT["amount"][k],resultDICT["sugar"][k],resultDICT["ice"][k])
                    responseSTR = responseSTR + "謝謝您的訂購\n"
                
                if "temperature" in resultDICT and "ice" not in resultDICT:
                    responseSTR = "Hi~~ \n您點的總共是：\n"
                    for k in range(0,len(resultDICT["amount"])):
                        responseSTR = responseSTR + "{} X {} ({}、{})\n".format(resultDICT["item"][k],resultDICT["amount"][k],resultDICT["sugar"][k],resultDICT["temperature"][k])
                    responseSTR = responseSTR + "謝謝您的訂購\n"   
